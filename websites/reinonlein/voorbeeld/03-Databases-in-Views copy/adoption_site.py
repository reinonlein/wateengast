...
class Puppy(db.Model):
    ...
    def __repr__(self):
        if self.owner:
            return f"Puppy name is {self.name} with ID {self.id} and owner is {self.owner.name}"
        else:
            return f"Puppy name is {self.name} with ID {self.id} and has no owner assigned yet."
...
def owner():
    ...
    if form.validate_on_submit():
        ...
        # Add new Puppy to database
        new_owner = Owner(ownername, pup_id)
