    ...
    return StreamBuilder<UserData>(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          ...
          double liters = ((stopuren / (7 * 24)) * userData.bier * 0.3).round() +
              ((stopuren / (7 * 24)) * userData.wijn * 0.125).round() +
              ((stopuren / (7 * 24)) * userData.sterk * 0.035);
                  ...
                  if (prestaties[index]['eenheid'] == 'uren') {
                    ...
                    return ListTile(
                      ...
                      subtitle: katers > prestaties[index]['target']
                          ? Text(
                              'Gehaald op ${Jiffy(Jiffy(userData.stopdate).add(months: stopmaanden.floor())).yMMMMd}',
                              style: TextStyle(color: Colors.grey),
                            )
                          : LinearProgressIndicator(
